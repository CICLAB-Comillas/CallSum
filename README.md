...
Since the conversations provided are in *.transcription* format, a pre-processing is required before the data can be ingested into the model. Typically, one of the most convenient formats for handling large amounts of text is CSV, since it allows storing information by fields, which in the case of conversations are 5: *Person, Transcribed line of dialogue, Number of dialogue line, Start time of dialogue line, End time of dialogue time and Quality of transcription*. To automate the process of reading, extraction and conversation, this *Preprocessing* module has been implemented.

The `preprocess.py` file allows to convert the different *.transcription* files into a single CSV, concatenating the dialogs of each line of the different transcriptions to form a single text string with the whole conversation. 

Before using it, it is good to know the different arguments (flags) received by the program:
...
* `-u`: Flag to indicate whether to generate a single CSV with all transcripts (default) or a CSV for each .transcription.
* `-e`: Input file extension. *.transcription* by default.

So the ncode for executing the preprocessing functionality is:
